...
class CRFTreeFeatureWriter:
    def __init__(self, verbose):
        ...
        self.cue_phrases = serialize.load_data('cue_phrases')
        if self.verbose:
...
#                print
    ...
    def write_dominance_set_features(self, L, R, position):
        ...
        if l_start_sent == l_end_sent:
            ...
            l_subtrees = get_syntactic_subtrees(t, l_start_word, l_end_word)
        ...
        if r_start_sent == r_end_sent:
            ...
            r_subtrees = get_syntactic_subtrees(t, r_start_word, r_end_word)
        ...
        for l_top_tags in l_subtrees_top_tags:
            for r_top_tags in r_subtrees_top_tags:
                distance = compute_edit_distance(l_top_tags, r_top_tags)
        ...
        for cue_phrase in self.cue_phrases:
            for (i, cand_span) in enumerate(candidates):
                ...
                if pos >= 0:
                    ...
                    # In Python 2, comparing between list of string against integer compares the name of the class (i.e., 'list' vs. 'int'). List of string is never <= int. We can just comment it out
